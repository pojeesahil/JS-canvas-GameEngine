   ...
   for(var tim=0;tim<arguments.length;tim++){
    ...
    tobj=[].concat(...arguments[tim]);
     ...
     function filt(tsrc){
      ...
      var timey=Math.ceil(canvas.height/game.tile.height);
      var timex=Math.ceil(canvas.width/game.tile.width);
      var startx=Math.floor(game.offset.x/game.tile.width);
      var starty=Math.floor(game.offset.y/game.tile.height);
      game.tile.cx=startx;
      game.tile.cy=starty;
      game.tile.cx2=startx+timex;
      game.tile.cy2=starty+timey;
    for(var timeyd=0;timeyd<timey;timeyd++){
      for(var timexd=0;timexd<timex;timexd++){
      game.display(game.tiles[starty+timeyd][startx+timexd]);
    }
    }
    }
    }catch(err){
...
var renderSystem={
  ...
  function makeObjectTileArray(sprite,arrx,arry){
    var tarr=[];
  for(var timed=0;timed<arry;timed++){
    tarr.push([]);
    for(var tim=0;tim<arrx;tim++){
      tarr[timed].push(sprite);
      if(typeof ext!==undefined){
        for(titem in ext){
          tarr[timed][tim][titem]=ext[titem];
        }
  ...
  function makeMovingObject(){
    ...
    function drawObject(){
     ...
     //  alert(typeof obj[timed].image);
       ...
       }else if(game.currentSystem==="life"){
          ctx.drawImage(obj[timed].type,obj[timed].x,obj[timed].y,obj[timed].width,obj[timed].height);
        }else{
          ctx.drawImage(obj[timed].type,game.tile.cx,obj[timed].y,game.tile.width,game.tile.height);
        }
